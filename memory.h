...
public:
	...
	template<typename T>
	static void readMemory(unsigned int baseAddress, T* buffer, size_t dataSize);
	...
	template<typename T>
	static void writeMemory(unsigned int baseAddress, T* buffer, size_t dataSize);
	
	/**DEBUG FUNCTIONS**/
	// print the contents of a memory block given a base address and size of block
	static void printMemBlock(unsigned int baseAddress, size_t size);
};
